    ...
    const fetchActivities = async () => {
      ...
      try {
        ...
        const { data, error } = await supabase
          .from('user_rsc') // Nome da sua tabela de atividades do usuário
          .select(`
            id,
            user_id,
            competence_id,
          ...
          `)
          .eq('user_id', currentUser.id); // Filtra atividades pelo ID do usuário logado
    ...
    activities.forEach(activity => {
      if (activity.status !== 'rejeitada') {
        ...
        if (item) {
          ...
          const categoryIndex = (categoryValue ? parseInt(categoryValue) : 0) - 1; // Converte para int e subtrai 1
